...
public class FileService {
    ...
    public ResponseResult delFileById(String id ) {
        if(StringUtils.isNotEmpty(id)){
            Optional<FileMetadata> file = fileMetadataRepo.findById(id);
            if (file.isPresent()) {
                ...
                gridFsTemplate.delete(new Query().addCriteria(Criteria.where("id").is(id)));
     ...
     * @return
     */
    public QueryResponseResult findList(int page, int size, FileMetadata queryPageRequest) {
        if (queryPageRequest == null) {
            queryPageRequest = new FileMetadata();
        }
        //定义条件匹配器
        ExampleMatcher exampleMatcher = ExampleMatcher.matching().withMatcher("fileOriginName", ExampleMatcher.GenericPropertyMatchers.contains());
        FileMetadata fileMetadata = new FileMetadata();
        if (StringUtils.isNotEmpty(queryPageRequest.getFileType())) {
            fileMetadata.setFileType(queryPageRequest.getFileType());
        }
        if (StringUtils.isNotEmpty(queryPageRequest.getFileOriginName())) {
            fileMetadata.setFileOriginName(queryPageRequest.getFileOriginName());
        }
        Example<FileMetadata> cm = Example.of(fileMetadata, exampleMatcher);
        ...
        Page<FileMetadata> all = fileMetadataRepo.findAll(cm, pageable);
